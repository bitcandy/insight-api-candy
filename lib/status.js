...
StatusController.prototype.show = function(req, res) {
  ...
  switch(option) {
    case 'getValuePools':
    this.getValuePools(function(err, result) {
      if (err) {
...
StatusController.prototype.getValuePools = function(callback) {
  this.node.services.bitcoind.getValuePools(function(err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, info);
