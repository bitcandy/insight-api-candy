...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  transformed.valueOut = transaction.outputSatoshis / 1e5;
  transformed.size = transaction.hex.length / 2; // in bytes
  if (!transaction.coinbase) {
    transformed.valueIn = transaction.inputSatoshis / 1e5;
    transformed.fees = transaction.feeSatoshis / 1e5;
  }
...
TxController.prototype.transformInput = function(options, input, index) {
  ...
  transformed.value = input.satoshis / 1e5;
  transformed.doubleSpentTxID = null; //5TODO
...
TxController.prototype.transformOutput = function(options, output, index) {
  var transformed = {
    value: (output.satoshis / 1e5).toFixed(5),
    n: index,
    scriptPubKey: {
      hex: output.script
    }
...
TxController.prototype.transformJoinSplit = function(options, jsdesc, index) {
  var transformed = {
    vpub_old: (jsdesc.oldZatoshis / 1e5).toFixed(5),
    vpub_new: (jsdesc.newZatoshis / 1e5).toFixed(5),
    n: index,
  };
  return transformed;
};
